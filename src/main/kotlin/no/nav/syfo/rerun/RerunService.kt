...
class RerunService(
    ...
    suspend fun start() {
        try {
            var stopTime = LocalDateTime.now().plusMinutes(5)
            var counter = 0
            while (applicationState.ready && LocalDateTime.now().isBefore(stopTime)) {
                val rerunMeldingerSomString = rerunConsumer.poll()
                rerunMeldingerSomString.forEach {
                    log.info("Behandler melding nr {}", ++counter)
                    behandleLegeerklaering(it)
                }
                if (rerunMeldingerSomString.isNotEmpty()) {
                    stopTime = LocalDateTime.now().plusSeconds(60)
                }
                delay(1)
            }
        } catch (ex: Exception) {
            log.info("Error processing rerun", ex)
        } finally {
            rerunConsumer.close()
        }
    }

    suspend fun behandleLegeerklaering(fellesformatText: String) {
        val fellesformat =
            fellesformatUnmarshaller.unmarshal(StringReader(fellesformatText)) as XMLEIFellesformat
            ...
            if (redisSha256String != null) {
                ...
                when (validationResult.status) {
                    Status.OK -> {
                        ...
                        log.info(
                            "Legeerklæring sendt til arena, til lokal kontornr: $lokaltNavkontor, {}",
                            fields(loggingMeta)
                        )
                        sendTilOKTopic(kafkaProducerLegeerklaeringSak, env.pale2OkTopic, legeerklaeringSak, loggingMeta)
                    }

                    Status.INVALID -> sendTilAvvistTopic(
