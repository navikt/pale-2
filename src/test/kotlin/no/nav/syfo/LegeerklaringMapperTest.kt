...
internal class LegeerklaringMapperTest {
    ...
    internal fun `Tester mapping fra fellesformat til Legeerklaring format`() {
        ...
        assertEquals(
            "Nyresvikt kronisk",
            legeerklaering.sykdomsopplysninger.bidiagnose.firstOrNull()?.tekst
        )
        assertEquals(
            LocalDateTime.of(2017, 11, 5, 0, 0, 0),
            legeerklaering.sykdomsopplysninger.arbeidsuforFra
        )
        assertEquals(
            "vethellerikkehvasomskalhit",
            legeerklaering.sykdomsopplysninger.sykdomshistorie
        )
        assertEquals("vetikkehvasomskalhit", legeerklaering.sykdomsopplysninger.statusPresens)
        assertEquals(
            false,
            legeerklaering.sykdomsopplysninger.borNavKontoretVurdereOmDetErEnYrkesskade
        )
        assertEquals(
            LocalDateTime.of(2017, 10, 14, 0, 0, 0),
            legeerklaering.sykdomsopplysninger.yrkesSkadeDato
        )
        ...
        assertEquals(
            "Burde dra ut p� fisketur for � slappe av",
            legeerklaering.plan?.utredningsplan
        )
        ...
        assertEquals(
            "Den gamle planen fungerte ikke",
            legeerklaering.plan?.narSporreOmNyeLegeopplysninger
        )
        assertEquals(
            "Trenger � slappe av med litt fisking",
            legeerklaering.plan?.videreBehandlingIkkeAktueltGrunn
        )
        ...
        assertEquals(
            "Kan ikke lengre danse",
            legeerklaering.funksjonsOgArbeidsevne.vurderingFunksjonsevne
        )
        ...
        assertEquals(
            false,
            legeerklaering.funksjonsOgArbeidsevne.kanGjenopptaTidligereArbeidEtterBehandling
        )
        assertEquals(
            "Ikke tunge l�ft",
            legeerklaering.funksjonsOgArbeidsevne.kanIkkeGjenopptaNaverendeArbeid
        )
